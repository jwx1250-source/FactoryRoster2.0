...
type IndustryOption = { id: string; name: string; parent_id: string | null; primary_industry: string; secondary_category: string | null };

export default function AdminFactoryForm({ initialFactory = {}, initialVerifications = [], initialContact = null, industries = [] }: { initialFactory?: Row; initialVerifications?: Verification[]; initialContact?: Row | null; industries?: IndustryOption[] }) {
  ...
  const primaryIndustries = useMemo(() => industries.filter((industry) => !industry.parent_id), [industries]);
  const secondaryCategories = useMemo(() => industries.filter((industry) => industry.parent_id === factory.industry_id), [factory.industry_id, industries]);
  ...
  return <form className="admin-form" onSubmit={(event) => save(event)}>
    ...
    <section className="admin-form-section"><h2>Supplier classification</h2><p className="admin-section-help">The evidence choice is restricted to evidence appropriate for the selected supplier type.</p><div className="admin-fields">
      <label className="admin-field"><FieldLabel label="Primary Industry" required /><select required value={display(factory.industry_id)} onChange={(event) => setFactory((current) => ({ ...current, industry_id: event.target.value || null, secondary_category_id: null }))}><option value="">Select a primary industry</option>{primaryIndustries.map((industry) => <option key={industry.id} value={industry.id}>{industry.name}</option>)}</select></label>
      <label className="admin-field"><FieldLabel label="Secondary Category" required /><select required disabled={!factory.industry_id} value={display(factory.secondary_category_id)} onChange={(event) => setFactory((current) => ({ ...current, secondary_category_id: event.target.value || null }))}><option value="">{factory.industry_id ? "Select a secondary category" : "Select a primary industry first"}</option>{secondaryCategories.map((category) => <option key={category.id} value={category.id}>{category.secondary_category ?? category.name}</option>)}</select></label>
