...
const sections = [
  ["Identity", [["company_name", "Company name", "text", true], ["chinese_name", "Chinese name", "text", false], ["factory_type", "Factory type", "text", false]]],
...
export default function AdminFactoryForm({ initialFactory = {}, initialVerifications = [], initialContact = null, industries = [] }: { initialFactory?: Row; initialVerifications?: Verification[]; initialContact?: Row | null; industries?: { id: string; name: string }[] }) {
  ...
  async function save(event: FormEvent, publish = false) {
    ...
    const payload = Object.fromEntries(Object.entries({ ...factory, is_published: publish || factory.is_published }).filter(([key]) => !["id", "slug", "record_id", "created_at", "updated_at", "has_verified_contact", "last_verified_at"].includes(key)).map(([key, value]) => [key, arrayFields.has(key) ? String(value ?? "").split(",").map((item) => item.trim()).filter(Boolean) : key === "established_year" && value ? Number(value) : value === "" ? null : value]));
    try {
  ...
  return <form className="admin-form" onSubmit={(event) => save(event)}>
    <p className="admin-form-help"><span className="admin-required">Required</span> fields are needed to save a factory. All other fields can be completed later.</p>
    ...
    {sections.map(([title, fields]) => <section className="admin-form-section" key={title}><h2>{title}</h2><div className="admin-fields">{fields.map(([key, label, type, required]) => <label className={`admin-field ${type === "textarea" ? "full" : ""}`} key={key}><FieldLabel label={label} required={required} />{type === "textarea" ? <textarea required={required} value={display(factory[key])} onChange={(event) => setFactory((current) => ({ ...current, [key]: event.target.value }))} /> : <input type={type} required={required} value={display(factory[key])} onChange={(event) => setFactory((current) => ({ ...current, [key]: event.target.value }))} />}</label>)}</div>{title === "Identity" && <label className="admin-field" style={{ marginTop: 14 }}><FieldLabel label="Industry" required /><select required value={display(factory.industry_id)} onChange={(event) => setFactory((current) => ({ ...current, industry_id: event.target.value || null }))}><option value="">Select an industry</option>{industries.map((industry) => <option key={industry.id} value={industry.id}>{industry.name}</option>)}</select></label>}</section>)}
    ...
    {id && <section className="admin-form-section"><h2>Verification checks</h2><p className="admin-section-help">Verification timestamps, verifier identity, and the factory last verified date are recorded automatically.</p>{checks.map((record, index) => <div className="admin-panel" key={record.verification_type}><h2>{labels[record.verification_type] ?? record.verification_type}</h2><div className="admin-fields"><label className="admin-field"><FieldLabel label="Status" required /><select value={record.status} onChange={(event) => updateCheck(index, "status", event.target.value)}><option value="pending">Pending</option><option value="verified">Verified</option><option value="failed">Failed</option></select></label><label className="admin-field"><FieldLabel label="Verification method" /><input value={display(record.verification_method)} onChange={(event) => updateCheck(index, "verification_method", event.target.value)} /></label><label className="admin-field full"><FieldLabel label="Checked items" /><div className="admin-actions">{Object.entries(record.checked_items ?? {}).map(([key, checked]) => <label key={key}><input type="checkbox" checked={checked} onChange={(event) => updateCheck(index, "checked_items", { ...record.checked_items, [key]: event.target.checked })} /> {key.replaceAll("_", " ")}</label>)}</div></label><label className="admin-field"><FieldLabel label="Evidence note" /><textarea value={display(record.evidence_note)} onChange={(event) => updateCheck(index, "evidence_note", event.target.value)} /></label><label className="admin-field"><FieldLabel label="Internal note" /><textarea value={display(record.internal_note)} onChange={(event) => updateCheck(index, "internal_note", event.target.value)} /></label></div></div>)}</section>}
