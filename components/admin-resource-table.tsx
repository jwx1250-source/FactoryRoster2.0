            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={String(row.id ?? row.event_id ?? index)}>
                  ...
                  <td><div className="admin-actions">{typeof row.factory_id === "string" && <Link href={`/admin/factories/${row.factory_id}`}>Factory</Link>}{workflow && <button className="admin-secondary" type="button" onClick={() => window.alert(JSON.stringify(row, null, 2))}>View</button>}{workflow && <button className="admin-secondary" type="button" onClick={() => updateRow(row, { internal_note: window.prompt("Internal note", String(row.internal_note ?? "")) ?? row.internal_note })}>Note</button>}{resource === "verification-requests" && <button className="admin-secondary" type="button" onClick={() => updateRow(row, { factory_id: window.prompt("Factory UUID", String(row.factory_id ?? "")) || null })}>Link factory</button>}{resource === "guides" && <Link href={`/admin/guides/${row.id}`}>Edit</Link>}{(resource === "contacts" || resource === "guides") && <button className="admin-danger" type="button" onClick={() => deleteRow(row)}>Delete</button>}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
